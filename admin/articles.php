...
<body>
    <div class="mob">
        ...
        <div class="container d-flex flex-column justify-content-center">
            ...
            <p class="text-center fs-4">Créer un nouvel article</p>
            <div class="container border shadow">
                ...
                <form class='d-flex fs-6' method='post' action='articles_management.php'>
                    <span class='col m-1 text-center'>Auto généré</span>
                    <input class='col input-group input-group-sm m-1' type='text' name='titre'>
                    <textarea class='col m-1 input-group input-group-lg' type='text' name='description'></textarea>
                    <input class='col m-1 input-group input-group-sm' type='text' name='photo' value='style/images/'>
                    <button class='col m-1 btn btn-sm btn-outline-danger' type='submit' name='ajouter'>Créer</button>

                </form>
            </div>
            <p class="text-center fs-4 mt-5">Modifier mes articles existants</p>
            <div class="container border shadow">
                <div class="d-flex text-center">
                    <span class="col-sm">N°</span>
                    <span class="col-sm">Titre</span>
                    <span class="col-sm">Description</span>
                ...
                <?php
                    ...
                    $sth = $dbh->prepare("SELECT * FROM article ORDER BY id, titre, description, photo DESC");
                    $sth->execute();
                    $result = $sth->fetchAll();

                    foreach ($result as $key => $value) {
                        echo("<form method='post' action='articles_management.php' class='d-flex fs-6'>");
                        ...
                        echo("<textarea class='col m-1 input-group input-group-lg' type='text' name='description'>".$value['description']."</textarea>");
